...
//Here we check security_key is ok or not, decryptEmail,decryptPassword length is bigger than 0 or not so that don't put empty entry
	...
	$connection = mysqli_connect("localhost", "db_mehedi04286", "Me72779673+*+*##", " db_userdata");
	$sqlCommand2 = "INSERT INTO db_userdata (email, name, password, image) VALUES ($decryptEmail, $name, $decryptpassword, $filePath)";
	$result2 = mysqli_query($connection, $sqlCommand2);
	if($result2){
	   echo 'Signup Success';
	}
	   else{
